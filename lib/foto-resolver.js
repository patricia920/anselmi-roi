...
(function (global) {
  ...
  // Reescreve URLs do Zenphoto (photo.anselmi.ind.br/Fotos/X.jpg) pro proxy
  // /api/foto?u=X.jpg do Cloudflare Pages Function. O proxy busca server-side
  // (sem hotlink-protection do Referer) e cacheia 1 dia.
  // Para URLs que não são do Zenphoto (ex: PLM image-resizer), passa direto.
  function _proxify(url) {
    if (!url || typeof url !== 'string') return url;
    const m = url.match(/^https?:\/\/photo\.anselmi\.ind\.br\/Fotos\/([^?#]+)$/);
    if (!m) return url;
    // v=2 busta o cache do edge — o Worker antigo cacheava 404 por 1h e o CF
    // continuava servindo STALE depois disso. Incrementar quando mudar política.
    return '/api/foto?u=' + encodeURIComponent(m[1]) + '&v=2';
  }
  function _proxifyList(arr) {
    return Array.isArray(arr) ? arr.map(_proxify) : arr;
  }
  ...
  function getFotosByRefColor(ref, color) {
    ...
    const raw = cores[c] || cores[color] || cores[String(color).toUpperCase()] || [];
    return _proxifyList(raw);
  }
  ...
  function getFotoAnyColor(ref) {
    ...
    for (const k of keys) {
      if (cores[k].length > 0) return _proxify(cores[k][0]);
    }
    return null;
  }

  // Retorna TODAS as URLs disponíveis no banco pra essa ref (todas cores,
  // todas posições). Ordem: itera Object.keys do dict de cores, e dentro de
  // cada cor pega todas as URLs. Pra usar no cascade onerror quando se quer
  // tentar várias cores em sequência antes de cair pra silhueta.
  function getAllFotos(ref) {
    if (!banco) return [];
    const cores = banco[ref] || banco[_padRef(ref)];
    if (!cores) return [];
    const out = [];
    for (const k of Object.keys(cores)) {
      for (const u of (cores[k] || [])) {
        out.push(_proxify(u));
      }
    }
    return out;
